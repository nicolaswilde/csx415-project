...
salaryPredTrain <- function(mplot=FALSE) {
    ...
    # train model: CART
    salary_model <- rpart(Conversion1718~Age+G+GS+MP+FG+FGA+FGP+X3P+X3PA+X3PP+X2P+X2PA+X2PP+eFGP+FT+FTA+FTP+ORB+DRB+TRB+AST+STL+BLK+TOV+PF+PTS, data=salary_data)
    # print(salary_model)
    if (mplot==TRUE) {
        plot(salary_model)
        text(salary_model, use.n=T)
    }
